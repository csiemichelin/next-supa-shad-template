...
export async function POST(request: Request) {
  try {
    ...
    if (categoryError || !category) {
      ...
      return NextResponse.json({ message: '找不到分類' }, { status: 404 })
    }

    console.log('Menu category folder id:', category.id)

    const arrayBuffer = await file.arrayBuffer()
    const buffer = Buffer.from(arrayBuffer)
    const fileExt = file.name.split('.').pop() ?? 'jpg'
    const baseName = file.name.replace(/\.[^/.]+$/, '').replace(/[^a-zA-Z0-9-_]/g, '')
    const fileName = `${baseName || 'menu'}-${Date.now()}-${Math.random()
      .toString(36)
      .slice(2)}.${fileExt}`
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const storagePath = extractMenuStoragePath(imageUrl)
    if (!storagePath) {
      return NextResponse.json({ message: '無效的圖片網址' }, { status: 400 })
    }

    const { error } = await supabase.storage.from('menu').remove([storagePath])
    if (error) {
